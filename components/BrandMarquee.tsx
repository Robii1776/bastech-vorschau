...
export function BrandMarquee({ dark = false }: { dark?: boolean }) {
  return (
    <section
      aria-label="Hersteller, mit denen wir vertraut sind"
      className={dark ? "border-t border-on-brand/10" : "border-y border-line bg-white"}
    >
      <div className="mx-auto flex max-w-[1320px] items-center gap-8 px-5 py-5 sm:px-8">
        <p className={`shrink-0 text-[13px] font-medium ${dark ? "text-on-brand/60" : "text-ink-soft"}`}>
          Vertraut mit
        </p>
        <div className="marquee flex-1" aria-hidden="true">
          <div className="marquee-track gap-12">
            {[...brands, ...brands].map((brand, i) => (
              <span
                key={`${brand}-${i}`}
                className={`whitespace-nowrap font-mono text-[13px] uppercase tracking-wider ${
                  dark ? "text-on-brand/55" : "text-ink-soft"
                }`}
              >
                {brand}
              </span>
